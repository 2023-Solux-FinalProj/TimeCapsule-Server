            ...
            connection.query(getWriterIDQuery, [writer], (err, userResult) => {
                ...
                connection.query(insertCapsuleQuery, [memberID, send_at, arrive_at, music, theme, sendstate], (err, capsuleResult) => {
                    ...
                    .then((data) => {
                      ...
                      connection.query(insertContentsQuery, [data], (err, contentResult) => {
                        if (err) {
    ...
    try {
      ...
      if (!capsuleIDResult || capsuleIDResult.length === 0) {
        ...
        const response2 = {
          ...
          email: email.toString(),
          name: username,
          capsules: capsuleResult.reduce((acc,capsule => {
            const existingCapsule=acc.find(item => item.id === capsule.capsuleID);
