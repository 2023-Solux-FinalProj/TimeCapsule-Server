    ...
    connection.query(
      ...
      function (error, results, fields) {
        ...
        if (results.length <= 0) {
          try {
            connection.query(
              `INSERT INTO User (email, birth, created_at, username) VALUES (?, ?, ?, ?)`,
  ...
  (req, res) => {
    ...
    connection.query(getWriterIDQuery, [writer], (err, userResult) => {
      ...
      const insertCapsuleQuery =
        'INSERT INTO Capsule (senderID, send_at, arrive_at, music, theme, sendState) VALUES (?, ?, ?, ?, ?, ?)';
      connection
        .query(
          insertCapsuleQuery,
          [memberID, send_at, arrive_at, music, theme, sendstate],
          (err, capsuleResult) => {
            if (err) {
            ...
            Promise.all(promises).then((data) => {
              const insertContentsQuery =
                'INSERT INTO Contents (capsuleID, imageUrl, text) VALUES ?';
              connection.query(
                insertContentsQuery,
                [data],
                (err, contentResult) => {
                  if (err) {
                    console.error(
                      'Error executing MySQL query (Contents):',
                      err
                    );
                    return res.status(500).json({
                      isSuccess: false,
                  ...
                  const insertReceiverQuery =
                    'INSERT INTO Receiver (capsuleID, toEmail) VALUES (?, ?)';
                  connection.query(
                    insertReceiverQuery,
                    [capsuleID, receiver],
                    (err, receiverResult) => {
                      if (err) {
                        console.error(
                          'Error executing MySQL query (Receiver):',
                          err
                        );
                        return res.status(500).json({
                          isSuccess: false,
  ...
  return new Promise((resolve, reject) => {
    ...
    s3.upload(params, function (err, data) {
      if (err) {
        ...
        resolve(data.Location); // 성공 시 URL을 resolve로 반환
      }
    });
  });
}
